...
public class DiscordMessageHandler {
  ...
  private void start(InteractionImmediateResponseBuilder responder, Optional<LPInstance> lpInstance, Optional<SlashCommandInteractionOption> customCountdown) {
    ...
    try {
      ...
      BotUtils.sendResponse(responder, countdown + " isn't a valid positive number");
      return;
    }
    ...
    linkFromCommand
      ...
      .ifPresent(link -> {
        ...
        if (registeredInstance != null) {
          BotUtils.sendResponse(responder, "Listening Party link set! Use `/start` to begin the session");
        } else {
          BotUtils.sendResponse(responder, "ERROR: Invalid Spotify album/playlist ID or a Listening Party is currently in progress!");
        }
      });
  }

  private void link(InteractionImmediateResponseBuilder responder, Optional<LPInstance> lpInstance) {
    lpInstance.ifPresentOrElse((lp) -> BotUtils.sendResponse(responder, lp.getAlbumOrPlaylistUri()), () -> sendGenericUnsetError(responder));
  }
      ...
      slashCommandInteraction.getArgumentAttachmentValueByName("attachment").ifPresent(attachment -> {
        if (!attachment.isImage()) {
          try {
            ...
            BotUtils.sendResponse(responder, "Invalid TOTW data format.");
            e.printStackTrace();
          }
        }
      });
    } else {
      BotUtils.sendResponse(responder, "Access denied.");
    }
  }
  ...
  private void sendGenericUnsetError(InteractionImmediateResponseBuilder responder) {
    BotUtils.sendResponse(responder, "There's currently no Listening Party set for this channel! Use `/set <link>` to set it");
  }
}
